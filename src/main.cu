...
int main(int argc, char * argv[]){
  ...
  cudaSetDevice(1); //For my local machine, you probably should set it to 0 or remove it
  ...
  for(int i = graphs.size()-1; i >= 0; i--){
    ...
    if (!big_graphs_needed[i]){
	  ...
	  //printf("No threads here - NUM BLOCKS:%d - NUM THREADS:%d \n" , NUM_BLOCKS, NUM_THREADS);
          BloomFilter_Insert<<<NUM_BLOCKS, NUM_THREADS, size_of_shared_array>>>(d_V, d_A, d_bloomFilter, graphs[i]->num_vertices, graphs[i]->num_edges);
          Embedding_Kernel<<<NUM_BLOCKS, NUM_THREADS, size_of_shared_array>>>(graphs[i]->num_vertices, batch_size_small_graphs, d_V, d_A, d_embeddings, dimension, negative_samples, learning_rate_ec, d_sigmoid_lookup_table, j, epoch_distribution[graphs.size() - 1 - i]/batch_size_small_graphs, g_alpha, wedge, bf, negative_weight, d_bloomFilter, WARP_SIZE, WARPS_PER_BLOCK, NUM_WARPS); //call for device
