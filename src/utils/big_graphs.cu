...
BigGraphs::BigGraphs(int _dimension, int _negative_samples, unsigned long long _epochs, double _learning_rate, int _epoch_batch_size, int _alpha, int _wedge, int _bf, double _negative_weight, int _lrd_strategy, emb_t* _h_embeddings, CSR<vid_t> *_csr, float* _d_sigmoid_lookup_table, int _num_parts_gpu, int _num_pools_gpu, int _concurrent_samplers, int _sampling_threads, int _num_pool_sets, int _task_queue_threads, int _device_id , unsigned int* _bloomFilter, unsigned int* _vertexList): dimension(_dimension), negative_samples(_negative_samples), epochs(_epochs), learning_rate(_learning_rate), epoch_batch_size(_epoch_batch_size), alpha(_alpha), wedge(_wedge), bf(_bf), negative_weight(_negative_weight), lrd_strategy(_lrd_strategy), h_embeddings(_h_embeddings), csr(_csr), d_sigmoid_lookup_table(_d_sigmoid_lookup_table), num_parts_gpu(_num_parts_gpu), num_pools_gpu(_num_pools_gpu), concurrent_samplers(_concurrent_samplers), sampling_threads(_sampling_threads), task_queue(_task_queue_threads, _device_id), num_pool_sets(_num_pool_sets), device_id(_device_id), bloomFilter(_bloomFilter), vertexList(_vertexList){
#ifdef MEASURE_TIME
  double start = omp_get_wtime();
#endif
