...
class BigGraphs{
  public:
    ...
    BigGraphs(int _dimension, int _negative_samples, unsigned long long _epochs, double _learning_rate, int _epoch_batch_size, int _alpha, int _wedge, int _bf, double _negative_weight, int _lrd_strategy, emb_t* _h_embeddings, CSR<vid_t> *_csr, float* _d_sigmoid_lookup_table, int _num_parts_gpu, int _num_pools_gpu, int _concurrent_samplers = 4, int sampling_threads = 16, int _num_pool_sets=2, int task_queue_threads = 8, int device_id=0 , unsigned int* _bloomFilter = nullptr, unsigned int* _vertexList = nullptr);
