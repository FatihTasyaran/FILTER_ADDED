...
*/
//These hash functions are used to hash strings, we need to hash integers
/*
__device__ unsigned int djb2Hash(unsigned int to_add, int tid) {
...
*/
/*
__device__ unsigned int sdbmHash(unsigned int to_add, int tid) {
    unsigned int hash = 0;
    unsigned int c = 0;
    while (str[start] != ',') {
        c = str[start];
...
__device__ float CountCommonAndTotalBits(unsigned int* vertexList, unsigned int vertex1, unsigned int vertex2, unsigned int* bloomFilter) {
  ...
  //Buraya birseyler yazıcaz
  
  float resultRatio = 0;//float resultRatio;
...
__global__ void BloomFilter_Insert(unsigned int* vertexList, unsigned int* edgeList, unsigned int* bloomFilter, int numVertices, int numEdges){
	...
	//This is where to add hash inserts
...
__device__ void single_sample_update(emb_t * vEmbeddings, emb_t * uEmbeddings, int dimensions, int positive, double learning_rate, int id, double bias, float* d_sigmoid_lookup_table, double negative_weight, int WARP_SIZE, unsigned int source, unsigned int generated_sample, unsigned int* bloomFilter, unsigned int * vertexList, int bf){
  ...
  // call for bloom filter
    ...
    f = (positive - fastSig(g, d_sigmoid_lookup_table)) * learning_rate * fake_jaccard;
  }
  else{
    //bloom filter off, so no fake jaccard, only original gosh calculation
...
__global__ void Embedding_Kernel(unsigned int d_num_vertices, int d_num_epoch, unsigned int * d_V, unsigned int * d_A,  emb_t * d_embeddings, int d_dim, int d_s, double d_lr, float* d_sigmoid_lookup_table, int ep_start, int total_batches, int alpha, int wedge, int bf, double negative_weight, unsigned int* bloomFilter, int WARP_SIZE, int WARPS_PER_BLOCK, int NUM_WARPS){
...
__global__ void Big_Graphs_Embedding_Kernel(emb_t *source_bin, emb_t* dest_bin, long long vertices_per_part, int num_vertices, int starting_ep, int batch_ep,vid_t* vids, double d_lr, int dim, int neg_s, float* sig_table, int alpha, int wedge, int bf, int source_part_id, int dest_part_id , int WARP_SIZE, int WARPS_PER_BLOCK, int NUM_WARPS,  unsigned int* bloomFilter, unsigned int* vertexList){
  const unsigned int id = threadIdx.x + blockIdx.x*blockDim.x;
  const unsigned int warp_num = id/WARP_SIZE;
  ...
  for (w = warp_num; w<size_s2d; w+=NUM_WARPS){
    ...
    if (!u_in_part_x(p_sample, dest_part_id, vertices_per_part, num_vertices))
      printf("PROBLEM: s2d - Found a positive sample while processing %d, %d that is incorrect\n", source_part_id, dest_part_id);
#endif
    single_sample_update(myemb_s, dest_bin+(p_sample-vertices_per_part*dest_part_id)*dim, dim, 1, lr, id, nce_bias, sig_table, 1, WARP_SIZE, source, p_sample, bloomFilter, vertexList, bf);
    for (int s = 0;s <neg_s; s++){
      ns = get_negative_sample(ns_limit, &mySeed);
      single_sample_update(myemb_s, dest_bin+ns*dim, dim, 0, lr, id, nce_neg_bias, sig_table, 1, WARP_SIZE, source, ns, bloomFilter, vertexList, bf);
    }
#ifdef SHARED_MEMORY
    for (int j =id%WARP_SIZE; j<dim; j+=WARP_SIZE)
      myemb_g[j] = myemb_s[j];
...
#endif
      single_sample_update(myemb_s, source_bin+(p_sample-vertices_per_part*source_part_id)*dim, dim, 1, lr, id, nce_bias, sig_table, 1, WARP_SIZE, source, p_sample, bloomFilter, vertexList, bf);
      for (int s = 0;s <neg_s; s++){
        ns = get_negative_sample(ns_limit, &mySeed);
        single_sample_update(myemb_s, source_bin+ns*dim, dim, 0, lr, id, nce_neg_bias, sig_table, 1, WARP_SIZE, source, ns, bloomFilter, vertexList, bf);
      }
#ifdef SHARED_MEMORY
      for (int j =id%WARP_SIZE; j<dim; j+=WARP_SIZE)
        myemb_g[j] = myemb_s[j];
